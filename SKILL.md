...
- 内容必须包含：隔夜美股收盘表现、盘前重大新闻/政策/事件、今日关注板块、今日关注个股
- 输出：markdown + JSON + API 上报（结构比当日复盘精简）
- 标题强制格式：`YYYY年M月D日早盘快报`
- 注意：早盘快报不包含当日盘中数据（尚未开盘）。输出「今日关注板块」和「今日关注个股」时，必须综合「昨日盘面表现 + 隔夜消息面」得出结论，不是单纯的消息罗列。
...
- **API 上报类型验证**：生成 JSON 后、上报前，必须逐字段核对类型是否符合下方类型表。常见错误：`markets` 写成数组、`todayHot` 设为 `null`、`changePercent` 为字符串。
- **输出前复查**：逐条核对：
...
- **JSON 字段类型**：完整字段表见 [`references/review_model.md`](./references/review_model.md)。常见类型错误（API 拒收原因 + 修复方法）见 [`references/review_api.md`](./references/review_api.md) 的「常见错误码与排错」章节。
