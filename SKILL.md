...
### 数据源约束
10. **以下渠道已确认不可用，严禁调用**：akshare 东财行业/概念板块（走的同一后端被封）、akshare THS 同花顺板块排名（同花顺改页面结构失效）。**2026-06-25 实测重要更新**：东财 `push2.eastmoney.com/api/qt/clist/get` 板块排名 API 实际**可用**——申万行业（`fs=m:90+t:2`）、概念板块（`fs=m:90+t:3`）、个股涨跌幅榜（`fs=m:0+t:6,m:0+t:13,m:0+t:80,m:1+t:2,m:1+t:23`）均能正常返回 200 OK，单次 ~3KB JSON 包含 100+ 板块精确涨跌幅 + 涨停家数统计。这是当日复盘板块数据的首选，**取代 skill 中反复声称的"已全面封杀"**。详见 [数据源参考](./references/data_source.md)「东财 push2 API 实测更新」章节。
...
### Sina /tob/ 收评文章 vs 东财 push2 API 的角色分工（2026-06-25 实战对比）
