...
- 数据源：
  ...
  - **韩股专用回退**：Naver Finance（`finance.naver.com/item/main.naver?code={6位代码}`），EUC-KR 编码，页面抓取。详见 `international-stock-prices` 技能。
  - ❌ **Finnhub 免费版不支持韩国 KRX 交易所**（返回 `"You don't have access to this resource."`），且 OTC/ADR 符号也无数据。韩股查询不要使用 Finnhub。
...
- **Finnhub 免费版查韩国 KRX 股票**（2026-06-24 验证）：返回 `"You don't have access to this resource."`，OTC ADR 也全为零。韩股用 yfinance 或 Naver Finance 即可

### Finnhub 免费版不支持韩国 KRX 交易所（2026-06-24 验证）

**症状**：Finnhub API 搜索能搜到 `005930.KS`（Samsung Electronics），但 quote 接口返回 `"error":"You don't have access to this resource."`。尝试 OTC 符号（`SSNLF`）和 ADR（`SSNGY`）均返回全零数据。

**根因**：Finnhub 免费版仅覆盖美股和部分欧洲市场，韩国 KRX 交易所数据需要付费订阅。

**解决办法**：
- 韩股优先使用 **yfinance**（`005930.KS` 格式）
- 若 Yahoo 被限频，回退到 **Naver Finance** 页面抓取（`finance.naver.com/item/main.naver?code=005930`），EUC-KR 编码，详见 `international-stock-prices` 技能
- 不要在 Finnhub 上浪费时间重试韩股查询
- **Finnhub 免费版查韩国 KRX 股票**（2026-06-24 验证）：返回 `"You don't have access to this resource."`，OTC ADR 也全为零。韩股用 yfinance 或 Naver Finance 即可
