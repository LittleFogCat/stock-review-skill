...
2. 收集板块排名与热点（CLS 侧边栏 / JRJ 首页 / Sina 标题；可选用 akshare THS 行业板块排名）。
3. 收集个股涨跌幅（腾讯 API 个股代码；可选用 pyTDX 个股日K）。
...
- 行业板块排名：akshare `stock_board_industry_summary_ths()`（同花顺）
...
2. **板块排名（领涨）**：用 akshare `stock_board_industry_summary_ths()`（同花顺数据源，不受东财阻断）。
...
**解决办法**：
- 行业板块排名：用 akshare `stock_board_industry_summary_ths()`（同花顺数据源，已验证稳定）
