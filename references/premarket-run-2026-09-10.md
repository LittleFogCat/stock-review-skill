...
## scorer.py 评分
- 用 `CODE,名称` 逗号分隔格式（冒号会静默返回0只）——2026-08-31 踩坑点再次验证
- 本次 10 只推荐中 8 只评分为负（仅招商轮船+1.20%/30%仓位、湖南黄金+0.08%/5.2%仓位）
- **R1（美伊冲突持续第3日）风险期下模型自然审慎**——多数评分<0 与「谨慎观察不建议新增仓位」基调一致，属正常而非异常
